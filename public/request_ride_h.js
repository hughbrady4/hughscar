...
function initApp() {
   ...
   // userMessage("Available Width: " + screen.availWidth);
...
function writeRequest(controlKey) {
   ...
   let rideRequestKey = firebase.database().ref("/ride-requests/" + controlKey);
   let updates = {};
   ...
   updates["/ride-requests/" + controlKey] = {
      rider_uid: user.uid,
      user_name: user.displayName,
      status: "Ready",
      startedAt: firebase.database.ServerValue.TIMESTAMP,
   ...
   updates["/ride-requests-by-user/" + user.uid + "/" + rideRequestKey.key] = {
      status: "Ready",
      user_name: user.displayName,
      startedAt: firebase.database.ServerValue.TIMESTAMP,
      request_date: document.getElementById("date").value,
